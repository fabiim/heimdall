...
IFlowReconcileListener, IInfoProvider {
    ...
    protected Device findDestByEntity(IEntityClass reference,
    	...
    	if (dstEntity.getMacAddress() != 0xffffffffffffL) {
    		Device device = primaryIndex.findDeviceByEntity(dstEntity);
    		return device;
    ...
    protected Device learnDeviceByEntity(Entity entity) {
        ...
        while (true) {
            ...
            // Look up the fully-qualified entity to see if it already
            // exists in the primary entity index.
            device = primaryIndex.findDeviceByEntity(entity);
            Long deviceKey = device!= null ? device.getDeviceKey() : null; 
            IEntityClass entityClass = null;
            ...
            if ((entityindex = device.entityIndex(entity)) >= 0) {
                ...
                if (!deviceMap.replace(device.getDeviceKey(), oldDevice, device)){
                	deviceMap.insert(device.getDeviceKey(), device); 
                	//continue; 
                }
                else{
